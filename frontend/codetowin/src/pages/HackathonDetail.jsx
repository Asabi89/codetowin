...
import '../assets/css/hackaton-detail.css';
...
export default function HackathonDetail() {
  ...
  const handleCopyInviteLink = (e) => {
    ...
    navigator.clipboard.writeText(linkText)
      ...
      .catch(() => {
        alert('Lien d\'invitation: ' + linkText);
      });
  };
  ...
  const handleSubmitProject = () => {
    if (!agreeGuidelines || !agreeTerms) {
      alert('Veuillez accepter les directives et conditions de service avant de soumettre.');
      return;
    }
    updateWorkspaceState({ submitted: true, previewActive: true });
    alert('Félicitations ! Votre projet a été soumis avec succès au Google Cloud Rapid Agent Hackathon.');
  };
                    ...
                    <div className="workspace-wrapper">
                      ...
                      {previewActive ? (
                        ...
                        <div id="workspace-editor-flow" style={{ display: 'block' }}>
                          {/* Steps Track */}
                          <div className="steps-indicator-track" role="tablist">
                            <button type="button" className={`step-indicator-item ${step === 1 ? 'is-active' : ''} ${step > 1 ? 'is-completed' : ''}`} onClick={() => handleJumpToStep(1)}>1. L'idée</button>
                            <button type="button" className={`step-indicator-item ${step === 2 ? 'is-active' : ''} ${step > 2 ? 'is-completed' : ''}`} onClick={() => handleJumpToStep(2)}>2. L'équipe</button>
                            <button type="button" className={`step-indicator-item ${step === 3 ? 'is-active' : ''} ${step > 3 ? 'is-completed' : ''}`} onClick={() => handleJumpToStep(3)}>3. Détails</button>
                            <button type="button" className={`step-indicator-item ${step === 4 ? 'is-active' : ''} ${step > 4 ? 'is-completed' : ''}`} onClick={() => handleJumpToStep(4)}>4. Questions</button>
                            <button type="button" className={`step-indicator-item ${step === 5 ? 'is-active' : ''} ${step > 5 ? 'is-completed' : ''}`} onClick={() => handleJumpToStep(5)}>5. Envoi</button>
                          </div>
              ...
              <div className="tab-screen active">
                <div className="section-slice">
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
                    ...
                    <a href="#new-post" onClick={(e) => { e.preventDefault(); alert("Bla-bla bientôt disponible !"); }} className="btn-action-primary">Nouveau truc à dire</a>
                  </div>
