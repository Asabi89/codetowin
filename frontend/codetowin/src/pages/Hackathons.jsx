...
// Default static hackathons dataset from the prototype
const initialHackathons = [
  {
    ...
    online: true,
    logoText: 'GC',
    keywords: 'google cloud rapid agent sprint online hybrid devs makers ai gemini mcp'
  },
  {
...
export default function Hackathons() {
  ...
  const getFilterDisplayValue = (group) => {
    ...
    if (val === 'all') {
      if (group === 'status') return 'Tous';
      if (group === 'location') return 'Partout';
      if (group === 'interest') return 'Tous';
    }
    // Return first word capitalized
    return val.split(/[,\s]+/)[0].toUpperCase().charAt(0) + val.split(/[,\s]+/)[0].slice(1).toLowerCase();
  };
  ...
  return (
    <div className="hackathon-page-wrapper">
      <section className="hero-strip" aria-label="Hackathon highlight">
        <p><strong>Les meilleurs hackathons tech</strong>, rejoins maintenant.</p>
      </section>
          ...
          {filteredHackathons.map((hackathon) => (
            ...
            >
              ...
              <div className="card-meta">
                ...
                <span className="date-range">{hackathon.id === 'google-cloud-rapid-agent' ? 'Juin 01 - Juin 12' : 'À venir'}</span>
                <span className="prize"><strong>{hackathon.prize}</strong> de prix</span>
              </div>
            </article>
          ))}
        </section>
